  ...
  def create
    if (params[:hall_seat][:row].to_i > @movie_hall.rows) or (params[:hall_seat][:row].to_i <= 0) or (params[:hall_seat][:col].to_i > @movie_hall.cols) or (params[:hall_seat][:col].to_i <= 0)
      respond_to do |format|
        format.html { redirect_to new_manage_cinema_cinema_movie_hall_hall_seat_path(@cinema, @movie_hall), :notice => "你输入的行号或者列号不符合要求！" }
      end
      return
    end
    ...
    respond_to do |format|
      if @hall_seat.save
        format.html { redirect_to manage_cinema_cinema_movie_hall_hall_seats_path(@cinema, @movie_hall), notice: 'Picture was successfully created.' }
